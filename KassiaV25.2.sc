...
// v25.2:
// - opens at a sensible default size (resizable)
// - "Fit window" button rebuilds layout to current window size
// - dynamic channel strip width
...
var initBtn, randBtn, stopBtn, fitBtn;
...
// Open at a sensible default — user can resize then hit "Fit window"
w = Window("Kassia", Rect(20, 20, 1600, 760)).front;
w.background_(dark);
...
// ------------------------------------------------------------------
// buildUI — called on startup and by the Fit window button.
// Tears down and rebuilds all views from current w.bounds.
// Synth state and morph routines are unaffected.
// ------------------------------------------------------------------

buildUI = {

	// Clear existing views
	if(top.notNil) { top.remove };
	if(mid.notNil)  { mid.remove };

	winW = w.bounds.width;
	winH = w.bounds.height;

	// Re-initialise per-strip arrays
	...
	morphRatioTo = { |targetRatio, dur = 10.0, updatesPerSecond = 20|
		...
		ratioMorphRoutine = Routine({
			steps.do { |i|
				...
				{ setRatioValue.(r); }.defer;
				waitTime.wait;
			};
			{ setRatioValue.(safeTarget); }.defer;
			ratioMorphRoutine = nil;
		}).play(AppClock);
	};
	...
	fitBtn = Button(top, Rect(1075, 8, 100, 24))
		.states_([["Fit window"]])
		.action_({ { buildUI.() }.defer(0.05); });

	randBtn = Button(top, Rect(1183, 8, 100, 24))
		.states_([["Rand phases"]])
		.action_({
	...
	stopBtn = Button(top, Rect(1291, 8, 100, 24))
		.states_([["Stop (fade)"]])
		.action_({
			stopRatioMorph.();
			~dr.free;
	...
	ratioGoBtn = Button(top, Rect(1222, 46, 72, 24))
		.states_([["Morph"]])
		.action_({ morphRatioTo.(ratioTargetNb.value, ratioTimeNb.value); });
	ratioStopBtn = Button(top, Rect(1300, 46, 96, 24))
		.states_([["Stop morph"]])
		.action_({ stopRatioMorph.(); });
	...
	// ----------------------------------------------------------------
	// Channel strips — width calculated from actual window bounds
	// ----------------------------------------------------------------

	startX = 8;
	stripW = ((winW - (startX * 2)) / ~dr.num).floor.asInteger;
	...
	// Restore readouts from current synth state
	syncCarrierRoot.();
	applyFMPartials.();
